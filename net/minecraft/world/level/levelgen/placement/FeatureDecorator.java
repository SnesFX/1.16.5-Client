...
public abstract class FeatureDecorator<DC extends DecoratorConfiguration> {
    ...
    private static <T extends DecoratorConfiguration, G extends FeatureDecorator<T>> G register(String string, G g) {
        return (G)Registry.register(Registry.DECORATOR, string, g);
    }

    public FeatureDecorator(Codec<DC> codec) {
        this.configuredCodec = codec.fieldOf("config").xmap(decoratorConfiguration -> new ConfiguredDecorator<DecoratorConfiguration>(this, (DecoratorConfiguration)decoratorConfiguration), ConfiguredDecorator::config).codec();
    }

    public ConfiguredDecorator<DC> configured(DC DC) {
        return new ConfiguredDecorator<DC>(this, DC);
    }
