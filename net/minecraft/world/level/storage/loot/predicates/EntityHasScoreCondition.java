...
implements LootItemCondition {
    ...
    implements net.minecraft.world.level.storage.loot.Serializer<EntityHasScoreCondition> {
        ...
        public EntityHasScoreCondition deserialize(JsonObject jsonObject, JsonDeserializationContext jsonDeserializationContext) {
            Set set = GsonHelper.getAsJsonObject(jsonObject, "scores").entrySet();
            LinkedHashMap linkedHashMap = Maps.newLinkedHashMap();
            for (Map.Entry entry : set) {
                linkedHashMap.put(entry.getKey(), GsonHelper.convertToObject((JsonElement)entry.getValue(), "score", jsonDeserializationContext, RandomValueBounds.class));
            }
            return new EntityHasScoreCondition(linkedHashMap, GsonHelper.getAsObject(jsonObject, "entity", jsonDeserializationContext, LootContext.EntityTarget.class));
        }
