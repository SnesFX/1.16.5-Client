...
public class PotionBrewing {
    ...
    protected static boolean isContainerIngredient(ItemStack itemStack) {
        ...
        for (int i = 0; i < n; ++i) {
            if (!CONTAINER_MIXES.get(i).ingredient.test(itemStack)) continue;
            return true;
        }
        return false;
    }
    ...
    protected static boolean isPotionIngredient(ItemStack itemStack) {
        ...
        for (int i = 0; i < n; ++i) {
            if (!POTION_MIXES.get(i).ingredient.test(itemStack)) continue;
            return true;
        }
        return false;
    }
    ...
    public static boolean isBrewablePotion(Potion potion) {
        int n = POTION_MIXES.size();
        for (int i = 0; i < n; ++i) {
            if (POTION_MIXES.get(i).to != potion) continue;
            return true;
        }
        return false;
    }
