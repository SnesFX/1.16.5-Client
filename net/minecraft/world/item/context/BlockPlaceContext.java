...
extends UseOnContext {
    ...
    public Direction[] getNearestLookingDirections() {
        int n;
        Direction[] arrdirection = Direction.orderedByNearest(this.getPlayer());
        if (this.replaceClicked) {
            return arrdirection;
        }
        Direction direction = this.getClickedFace();
        for (n = 0; n < arrdirection.length && arrdirection[n] != direction.getOpposite(); ++n) {
        }
        if (n > 0) {
            System.arraycopy(arrdirection, 0, arrdirection, 1, n);
            arrdirection[0] = direction.getOpposite();
        }
        return arrdirection;
    }
}
