...
IdMap<T> {
    ...
    protected static final WritableRegistry<WritableRegistry<?>> WRITABLE_REGISTRY = new MappedRegistry<WritableRegistry<?>>(Registry.createRegistryKey("root"), Lifecycle.experimental());
    ...
    public static <T extends WritableRegistry<?>> void checkRegistry(WritableRegistry<T> writableRegistry) {
        writableRegistry.forEach(writableRegistry2 -> {
            if (writableRegistry2.keySet().isEmpty()) {
                LOGGER.error("Registry '{}' was empty after loading", (Object)writableRegistry.getKey(writableRegistry2));
                if (SharedConstants.IS_RUNNING_IN_IDE) {
                    throw new IllegalStateException("Registry: '" + writableRegistry.getKey(writableRegistry2) + "' is empty, not allowed, fix me!");
                }
            }
            if (writableRegistry2 instanceof DefaultedRegistry) {
                ...
                Validate.notNull(writableRegistry2.get(resourceLocation), (String)("Missing default of DefaultedMappedRegistry: " + resourceLocation), (Object[])new Object[0]);
            }
        });
    }
    ...
    public <U> DataResult<Pair<T, U>> decode(DynamicOps<U> dynamicOps, U u) {
        if (dynamicOps.compressMaps()) {
            return dynamicOps.getNumberValue(u).flatMap(number -> {
                ...
                return DataResult.success((Object)object, (Lifecycle)this.lifecycle(object));
            }).map(object -> Pair.of((Object)object, (Object)dynamicOps.empty()));
        }
        return ResourceLocation.CODEC.decode(dynamicOps, u).flatMap(pair -> {
            T t = this.get((ResourceLocation)pair.getFirst());
            if (t == null) {
                return DataResult.error((String)("Unknown registry key: " + pair.getFirst()));
            }
            return DataResult.success((Object)Pair.of(t, (Object)pair.getSecond()), (Lifecycle)this.lifecycle(t));
        });
    }
    ...
    public <U> DataResult<U> encode(T t, DynamicOps<U> dynamicOps, U u) {
        ...
        if (dynamicOps.compressMaps()) {
            return dynamicOps.mergeToPrimitive(u, dynamicOps.createInt(this.getId(t))).setLifecycle(this.lifecycle);
        }
        return dynamicOps.mergeToPrimitive(u, dynamicOps.createString(resourceLocation.toString())).setLifecycle(this.lifecycle);
    }
    ...
    public static <V, T extends V> T registerMapping(Registry<V> registry, int n, String string, T t) {
        return ((WritableRegistry)registry).registerMapping(n, ResourceKey.create(registry.key, new ResourceLocation(string)), t, Lifecycle.stable());
    }
