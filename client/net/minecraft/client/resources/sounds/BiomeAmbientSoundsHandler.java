...
implements AmbientSoundHandler {
    ...
    public void tick() {
        ...
        if (biome != this.previousBiome) {
            ...
            biome.getAmbientLoop().ifPresent(soundEvent -> {
                LoopSoundInstance cfr_ignored_0 = (LoopSoundInstance)this.loopSounds.compute((Object)biome, (biome, loopSoundInstance) -> {
        ...
        this.moodSettings.ifPresent(ambientMoodSettings -> {
            ...
            BlockPos blockPos = new BlockPos(this.player.getX() + (double)this.random.nextInt(n) - (double)ambientMoodSettings.getBlockSearchExtent(), this.player.getEyeY() + (double)this.random.nextInt(n) - (double)ambientMoodSettings.getBlockSearchExtent(), this.player.getZ() + (double)this.random.nextInt(n) - (double)ambientMoodSettings.getBlockSearchExtent());
            int n2 = level.getBrightness(LightLayer.SKY, blockPos);
            this.moodiness = n2 > 0 ? (this.moodiness -= (float)n2 / (float)level.getMaxLightLevel() * 0.001f) : (this.moodiness -= (float)(level.getBrightness(LightLayer.BLOCK, blockPos) - 1) / (float)ambientMoodSettings.getTickDelay());
            if (this.moodiness >= 1.0f) {
                ...
                SimpleSoundInstance simpleSoundInstance = SimpleSoundInstance.forAmbientMood(ambientMoodSettings.getSoundEvent(), this.player.getX() + d4 / d7 * d8, this.player.getEyeY() + d5 / d7 * d8, this.player.getZ() + d6 / d7 * d8);
