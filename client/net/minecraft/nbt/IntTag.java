...
extends NumericTag {
    public static final TagType<IntTag> TYPE = new TagType<IntTag>(){
        ...
        @Override
        public IntTag load(DataInput dataInput, int n, NbtAccounter nbtAccounter) throws IOException {
    ...
    private IntTag(int n) {
        this.data = n;
    }

    public static IntTag valueOf(int n) {
        if (n >= -128 && n <= 1024) {
            return Cache.cache[n + 128];
        }
        return new IntTag(n);
    }
